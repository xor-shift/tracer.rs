...
@group(1) @binding(6) var<storage, read> geometry_buffer: array<GeometryElement>;
...
@fragment fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    //return vec4<f32>(geometry_buffer[gb_idx_u(tex_pos)].albedo.xyz, 1.);
    //return vec4<f32>(abs(geometry_buffer[gb_idx_u(tex_pos)].normal_and_depth.xyz), 1.);
}
